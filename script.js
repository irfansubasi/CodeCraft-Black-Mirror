...
const acceptModal = async () => {
    ...
    if(clickCount === 0) {
        ...
        overlay.style.backgroundImage = 'url(./assets/crack1.png)';
        const messageP = modal.querySelector('p');
        const messageText = messages[0];
        messageP.innerHTML = messageText.replace('everything', '<span class="neon-text">everything</span>');
        ...
        overlay.style.backgroundImage = 'url(./assets/crack2.png)';
        modal.querySelector('p').innerHTML = messages[1].replace('Nothing', '<span class="neon-text">Nothing</span>');

        setTimeout(() => {
            const tempText = document.createElement('p');
            tempText.id = 'tempText';
            tempText.innerHTML = messages[2].replace('you', '<span class="neon-text">you</span>');
        ...
        tempText.remove();
        overlay.style.backgroundImage = 'url(./assets/crack3.png)';
        modal.querySelector('p').innerHTML = messages[3].replace('Accept', '<span class="neon-text">Accept</span>');
    } else{
        modal.close();
